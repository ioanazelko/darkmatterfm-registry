...
| `registry/darkmatterwiki.html` | Browse and search every card with a full detail pane: facets by family, model kind, status and portal, plus summary counts. The page embeds the 3.7 MB card index and fetches each card's detail subtree on demand from `registry/darkmatterwiki_data/` (256 JSON shards, 59 MB in total), so it must be served over HTTP; `registry/build_viewers.py --hosted` builds it. Its navigation bar links the companion viewers `paper_status.html` (triage outcomes per paper), `ranking.html` (priority scores and tiers), `enrichment_status.html` and `enrichment_outputs.html` (the enrichment pass per model). |
